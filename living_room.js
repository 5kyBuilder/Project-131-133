...
function preload()
{
    ImageS = loadImage("living_room.jpg");
}
...
function draw()
{
    ...
    for(var i = 1; i<=objects.length; i++) // first object, i = 1. second object, i = 2. length = 2
    {
        ...
        if(objects[i - 1].label == "couch"){
            ...
            text(objects[i - 1].label + " " + percentage + "%", objects[i - 1].x + 35, objects[i - 1].y + 115);
        rect(objects[i - 1].x + 45, objects[i - 1].y + 155, objects[i - 1].width + 20, objects[i - 1].height);
        }
    }
}
